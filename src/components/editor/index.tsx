...
export default function Editor() {
  ...
  const footer = (
    <Box
      component="footer"
      sx={{ display: 'flex', bgcolor: 'background.default', borderTop: 1, borderColor: 'divider', px: 3, py: 1.5, alignItems: 'center', justifyContent: 'center', gap: 1.5, flexShrink: 0 }}
    >
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh', bgcolor: 'background.default', minWidth: device === 'pc' ? 960 : device === 'tablet' ? 600 : 320 }}>
      {/* Header */}
      <AppBar position="static" color="inherit" elevation={0} sx={{ borderBottom: 1, borderColor: 'divider', bgcolor: 'background.default' }} className="landscape-header">
        <MuiToolbar variant="dense" sx={{ justifyContent: 'space-between', px: device === 'mobile' ? 2 : 3 }}>
      ...
      {/* 탭 바 (모바일 전용) */}
      {device === 'mobile' && (
        <Box sx={{ flexShrink: 0 }} className="landscape-hide">
          <Tabs
            value={mobileTab}
      ...
      {/* ── 메인 콘텐츠 ── */}
      <Box sx={{ flex: 1, minHeight: 0, overflow: device === 'tablet' ? 'auto' : 'hidden', position: 'relative', display: device === 'pc' ? 'flex' : 'block' }}>
          ...
          <>
            ...
            <Box
              sx={{
                ...
                visibility: isDesign ? 'visible' : 'hidden',
                pointerEvents: isDesign ? 'auto' : 'none',
              }}
            ...
            <Box
              sx={{
                position: 'absolute',
                inset: 0,
                display: !isDesign ? 'flex' : 'block',
                flexDirection: 'column',
                visibility: !isDesign ? 'visible' : 'hidden',
                pointerEvents: !isDesign ? 'auto' : 'none',
                bgcolor: !isDesign ? 'background.paper' : 'transparent',
              }}
            >
        ...
        {device === 'tablet' && (
          <Box sx={{ display: 'flex', flexDirection: 'column', bgcolor: 'background.paper' }}>
            {/* 가로 도구모음 + 모양 셀렉터 */}
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 1.5, borderBottom: 1, borderColor: 'divider', overflowX: 'auto', flexShrink: 0 }}>
            ...
            {/* 3D 미리보기 */}
            <Box sx={{ height: 450, flexShrink: 0, p: 2 }}>
              {preview3D}
            </Box>
          ...
          <>
            ...
            >
              <Box sx={{ display: 'flex', flex: 1, minHeight: 0 }}>
                {/* 세로 툴바 */}
                <Box sx={{ display: 'flex', width: 64, flexShrink: 0, borderRight: 1, borderColor: 'divider', p: 1.5, bgcolor: 'grey.50', flexDirection: 'column', alignItems: 'center' }}>
            ...
            >
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: 1, borderColor: 'divider', p: 2, bgcolor: 'background.paper', flexShrink: 0 }}>
                <Box>
      ...
      {device === 'mobile' && (
        ...
        >
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 1.5, py: 0.75, borderTop: 1, borderColor: 'divider' }}>
            <Typography variant="caption" color="text.secondary" fontWeight={500} sx={{ fontSize: '0.65rem' }}>Seoyoon Park</Typography>
            <Typography color="text.disabled" sx={{ fontSize: '0.65rem' }}>·</Typography>
            <Typography
              component="a"
              href="mailto:dev.yelee@gmail.com"
              variant="caption"
              sx={{ fontSize: '0.65rem', color: 'primary.main', '&:hover': { color: 'primary.dark' }, transition: 'color 0.2s', textDecoration: 'none' }}
            >
              dev.yelee@gmail.com
            </Typography>
          </Box>
